...
# unet 네트워크 구축, nn module 을 U-net 클래스에 상속
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # image regression ->
        # residual learning = 네트워크 input, 네트워크 output 의 최종합
        # input 과 label 간 차이만을 학습

        # Denoising = noise part 만 학습
        # Super_resolution = High freq. 만 학습
        # Inpainting = 샘플링 되지 않은 부분만 학습
...
# SRResNet 의 generator part
class SRResNet(nn.Module):
    def __init__(self, in_channels, out_channels,
                 nker=64, learning_type="plain",
                 norm="bnorm", nblk=16):
        # nblk = number of block, resblock 이 몇번 반복될지 정의,
        super(SRResNet, self).__init__()

        self.learning_type = learning_type

        self.enc = CBR2d(in_channels, nker, kernel_size=9, stride=1,
                         padding=4, bias=True, norm=None, relu=0.0)
                         #padding = 커널 사이즈를 2로 나눈 후, 버림한 값

        res = []

        for i in range(nblk):
            res += [ResBlock(nker, nker,
        ...
        self.dec = CBR2d(nker, nker,
                         kernel_size=3, stride=1,
                         padding=1, bias=True,
                         norm=norm, relu=None)

        ps1 = []
        ps1 += [nn.Conv2d(in_channels=nker, out_channels=4 * nker,
        ...
        ps2 = []
        ps2 += [nn.Conv2d(in_channels=nker, out_channels=4 * nker,
                          kernel_size=3, stride=1, padding=1)]
...
class ResNet(nn.Module):
    def __init__(self, in_channels, out_channels,
                 nker=64, learning_type="plain", norm="bnorm", nblk=16):
        super(ResNet, self).__init__()
        self.learning_type = learning_type

        self.enc = CBR2d(in_channels=in_channels, out_channels=nker, kernel_size=3, stride=1,
